...
# Normal condition: RMSE=1.95, mean≈1.5, p99≈3.1
# Use higher alpha/beta ratio for more concentrated distribution (steeper curve)
normal_data = generate_data(mean=1.5, min_val=0.35, max_val=4.0, p99=3.1, alpha=3, beta=4, size=10000)

# Under-Exposure: worse than normal, but better than over
# mean≈1.85, p99≈3.5, medium spread
under_data = generate_data(mean=2.15, min_val=0.75, max_val=4.5, p99=4.2, alpha=2, beta=3.5, size=10000)

# Over-Exposure: worst performance, significantly worse
# mean≈2.3, p99≈4.2, more spread out with longer tail (flatter curve)
over_data = generate_data(mean=2.8, min_val=0.85, max_val=5.8, p99=5.2, alpha=1.5, beta=2.5, size=10000)
...
# Plot CDF
x_normal, y_normal = calc_cdf(normal_data)
x_under, y_under = calc_cdf(under_data)
x_over, y_over = calc_cdf(over_data)

plt.plot(x_normal, y_normal, '-', zorder=2, lw=2, color='#b21700', label='Normal')
plt.plot(x_under, y_under, '--', zorder=2, lw=2, color='#1D485D', label='Under-Exposure')
plt.plot(x_over, y_over, '-.', zorder=2, lw=2, color='#2F851B', label='Over-Exposure')
...
for data, x, y, color in [(normal_data, x_normal, y_normal, '#b21700'), 
                         (under_data, x_under, y_under, '#1D485D'),
                         (over_data, x_over, y_over, '#2F851B')]:
    mean = np.mean(data)
...
# Save as PDF
pp = PdfPages('./rob_accuracy_light_CDF.pdf')
pp.savefig()
pp.close()
